...
=LiteRef= comes with the =install.sh= script, which uses the =apt= manager to install all the dependencies of the server. In particular, it installs the =pip= manager and uses the latter to install all the needed =Python= libraries. So, simply run this script to install the system. =LiteRef= installs the needed Emacs packages automatically as well.
...
@@html:<a name="api">@@
The documentation follows =Emacs Lisp= conventions:
...
The following is some of the =Emacs Lisp= terminology used throughout
